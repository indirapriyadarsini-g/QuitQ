...
@Query("select o.id,o.status,op.quantity,op.amountPayable,op.totalAmount,op.discount,p.title,p.price,p.discount from OrderProduct op join op.product p join op.order o where p.v.user.username=?1")
